...
namespace Voidstrap.UI.Elements.Settings.Pages
{
    ...
    "https://clientsettings.roblox.com/v2/settings/application/PCDesktopClient"
        ...
        private async void LoadPreset_Click(object sender, RoutedEventArgs e)
        {
            ...
            var presetContent = dialog.SelectedPresetContent;
            if (presetContent == null || presetContent.Count == 0)
                return;

            // Clear existing flags if requested
            if (dialog.ClearExisting)
            {
                var keys = App.FastFlags.Prop.Keys.ToList();
            ...
            // Load preset flags
            foreach (var pair in presetContent)
            {
                if (pair.Value == null)
                    continue;
            ...
            // Refresh the UI
            ClearSearch();
            Frontend.ShowMessageBox($"Preset loaded successfully! {presetContent.Count} flags applied.", MessageBoxImage.Information);
        }
        ...
        private async void ImportJSONButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    ImportJSON(json);
                    Frontend.ShowMessageBox("JSON imported successfully!", MessageBoxImage.Information);
                }
                catch (Exception ex)
                {
                    Frontend.ShowMessageBox($"Error importing JSON: {ex.Message}", MessageBoxImage.Error);
                }
            }
        }
        ...
        private async void BulkEdit_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (change.Value == "DELETE")
                {
                    App.FastFlags.SetValue(change.Key, null);
                }
                else if (change.Key != change.Value && !App.FastFlags.Prop.ContainsKey(change.Value))
                {
                    // Rename operation
                    var oldValue = App.FastFlags.GetValue(change.Key);
                    App.FastFlags.SetValue(change.Key, null);
                    App.FastFlags.SetValue(change.Value, oldValue);
                }
                else
                {
                    // Value change operation
                    App.FastFlags.SetValue(change.Key, change.Value);
                }
            }
            ...
            await LoadKnownFlagsAsync();
            UpdateExistsColumn();
            Frontend.ShowMessageBox($"Bulk edit completed! {changes.Count} changes applied.", MessageBoxImage.Information);
        }

        private void ComparePresets_Click(object sender, RoutedEventArgs e)
        {
            var dialog = new ComparePresetsDialog();
            dialog.ShowDialog();
        }
